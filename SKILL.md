...
tools:
  ...
  - name: lemonade_pull_model
    ...
    parameters:
      server_url: string
      model: string
    command: python3 manager.py pull --url "{{server_url}}" --model "{{model}}"
  ...
  - name: lemonade_load_model
    ...
    parameters:
      server_url: string
      model: string
    command: python3 manager.py load --url "{{server_url}}" --model "{{model}}"
  ...
  - name: lemonade_unload_model
    ...
    parameters:
      server_url: string
      model: string
    command: python3 manager.py unload --url "{{server_url}}" --model "{{model}}"
  ...
  - name: lemonade_chat_completion
    ...
    command: python3 manager.py chat --url "{{server_url}}" --model "{{model}}" --messages '{{messages_json}}'
  ...
  - name: lemonade_generate_image
    ...
    parameters:
      ...
      model: string
      prompt: string
    command: python3 manager.py image --url "{{server_url}}" --model "{{model}}" --prompt "{{prompt}}"
---
...
2. **Hardware & Resource Awareness:** * Local models demand significant GPU/NPU resources. Before initiating any heavy load, use `lemonade_check_health` on the target server.
